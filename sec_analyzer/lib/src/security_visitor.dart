  ...
  bool visitSimpleIdentifier(SimpleIdentifier node) {
    if (node.inGetterContext()){
      if( node.staticElement is ParameterElement ||
        ...
        if(node.staticElement is FunctionElement && node.staticElement.library.name.contains("dart.core")){
          //read annotation from dsl file
          node.setProperty(SEC_TYPE_PROPERTY,ExternalLibraryAnnotations.getSecTypeForFunction(node.name));
        }
        else {//resolved type by this visitor
      ...
      if(node.staticElement is LocalVariableElement){
        var referedNode  = node.staticElement.computeNode();
        node.setProperty(SEC_TYPE_PROPERTY, referedNode.getProperty(SEC_TYPE_PROPERTY));
      }
    }
    return true;
  }
...
class ExternalLibraryAnnotations{
  static SecurityType getSecTypeForFunction(String name){
    if(name == "print")
      return new SecurityFunctionType(new LowLabel(),
